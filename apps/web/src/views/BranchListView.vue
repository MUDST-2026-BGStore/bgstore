...
const debouncedSearchQuery = useDebounced(rawSearchQuery);
...
const searchQuery = computed(() =>
  debouncedSearchQuery.value.trim().toLowerCase(),
);
...
function isBranchBookable(branch: Branch): boolean {
  return branch.status !== 'INACTIVE';
}

function formatHours(branch: Branch): string {
...
function calculateDistance(branch: Branch): string | null {
  if (
    !userCoords.value ||
