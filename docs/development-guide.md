...
| Database tables or indexes              | The `@Entity` classes in the owning module         | `spring.jpa.hibernate.ddl-auto` in `application.properties`  |
...
5. Edit the `@Entity` class for schema changes; `ddl-auto=update` creates the table or adds the column at startup. It is additive only, so a rename, a drop, or a type change needs a hand-applied statement in every shared environment. Reference rows belong in a seeder such as `BranchDirectorySeed`, not in the schema.
